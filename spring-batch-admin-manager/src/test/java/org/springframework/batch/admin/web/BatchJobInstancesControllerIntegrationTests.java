...
public class BatchJobInstancesControllerIntegrationTests extends AbstractControllerIntegrationTest {
    ...
    public void testGetJobInstanceByInstanceId() throws Exception {
        ...
        when(jobService.getJobExecutionsForJobInstance(jobInstance.getJobName(), jobInstance.getId()))
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.jobInstanceInfoResource.jobExecutions[0].stepExecutions[0].stepName").value("s1"));
    }

    @Test
    public void testGetJobInstanceByJobName() throws Exception {
        when(jobService.listJobInstances("job1", 0, 20))
                .thenReturn(Arrays.asList(jobInstance, new JobInstance(3l, "job1")));
        when(jobService.getJobExecutionsForJobInstance(jobInstance.getJobName(), jobInstance.getId()))
                .thenReturn(Arrays.asList(execution));

        mockMvc.perform(
                get("/batch/instances").param("jobname", "job1").param("startJobInstance", "0").param("pageSize", "20")
                        .accept(
                                MediaType.APPLICATION_JSON))
                .andDo(print()).andExpect(status().isOk())
                .andExpect(jsonPath("$.pagedModel.content", Matchers.hasSize(2)))
                .andExpect(jsonPath("$.pagedModel.content[*].instanceId", contains(0, 3)))
                .andExpect(jsonPath("$.pagedModel.content[*].jobName", contains("job1", "job1")));
    }

    @Test
    public void testGetJobInstanceByInvalidInstanceId() throws Exception {
        when(jobService.getJobInstance(100l))
                .thenThrow(new NoSuchJobInstanceException("Batch Job instance with the id 100 doesn't exist"));

        mockMvc.perform(
                get("/batch/instances/100").accept(MediaType.APPLICATION_JSON)).andExpect(status().isNotFound())
                .andExpect(
                        jsonPath("$.message", Matchers.is("Batch Job instance with the id 100 doesn't exist")));
    }
}
