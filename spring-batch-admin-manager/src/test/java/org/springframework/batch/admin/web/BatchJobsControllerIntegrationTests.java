...
public class BatchJobsControllerIntegrationTests extends AbstractControllerIntegrationTest {
    ...
    public void testGetBatchJobs() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andDo(print()).andExpect(
                        status().isOk())
                .andExpect(jsonPath("$.pagedModel.content", Matchers.hasSize(2))).andExpect(
                        jsonPath("$.pagedModel.content[*].executionCount", contains(2, 1)))
                .andExpect(
                        jsonPath("$.pagedModel.content[*].launchable", contains(false, true)))
                .andExpect(
                        jsonPath("$.pagedModel.content[*].incrementable", contains(false, true)))
                .andExpect(
                        jsonPath("$.pagedModel.content[*].jobInstanceId", contains(nullValue(), nullValue())))
                .andExpect(
                        jsonPath("$.pagedModel.content[*].startTime", contains("1970-01-01T00:00:01.000Z", null)))
                .andExpect(
                        jsonPath("$.pagedModel.content[*].endTime", contains("1970-01-01T00:00:02.000Z", null)))
                .andExpect(
                        jsonPath("$.pagedModel.content[*].stepExecutionCount",
                                contains(info.getStepExecutionCount(), 0)))
                .andExpect(
                        jsonPath("$.pagedModel.content[*].jobParameters.parameters['foo'].value", contains("bar")))

                // should contain the display name (ie- without the .job suffix)
                .andExpect(jsonPath("$.pagedModel.content[0].name", equalTo("job1"))).andExpect(
                        jsonPath("$.pagedModel.content[0].jobInstanceId", nullValue()))

                .andExpect(jsonPath("$.pagedModel.content[1].name", equalTo("job2"))).andExpect(
                        jsonPath("$.pagedModel.content[1].jobInstanceId", nullValue()))

                // exit status is non null for job 0 and null for job 1
                .andExpect(
                        jsonPath("$.pagedModel.content[0].exitStatus.exitDescription",
                                equalTo(execution.getExitStatus().getExitDescription())))
                .andExpect(
                        jsonPath("$.pagedModel.content[0].exitStatus.exitCode",
                                equalTo(execution.getExitStatus().getExitCode())))
                .andExpect(
                        jsonPath("$.pagedModel.content[0].exitStatus.running", equalTo(false)))
                .andExpect(
                        jsonPath("$.pagedModel.content[1].exitStatus", nullValue()));
    }
    ...
    public void testGetPagedBatchJobs() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andDo(print()).andExpect(
                        status().isOk())
                .andExpect(jsonPath("$.pagedModel.content", Matchers.hasSize(1))).andExpect(
                        jsonPath("$.pagedModel.content[*].executionCount", contains(2)))
                .andExpect(
                        jsonPath("$.pagedModel.content[*].launchable", contains(false)))
                .andExpect(
                        jsonPath("$.pagedModel.content[*].incrementable", contains(false)))
                .andExpect(
                        jsonPath("$.pagedModel.content[*].jobInstanceId", contains(nullValue())))
                .andExpect(
                        jsonPath("$.pagedModel.content[*].startTime", contains("1970-01-01T00:00:01.000Z")))
                .andExpect(
                        jsonPath("$.pagedModel.content[*].endTime", contains("1970-01-01T00:00:02.000Z")))
                .andExpect(
                        jsonPath("$.pagedModel.content[*].stepExecutionCount", contains(info.getStepExecutionCount())))
                .andExpect(
                        jsonPath("$.pagedModel.content[*].jobParameters.parameters['foo'].value", contains("bar")))
                .andExpect(
                        jsonPath("$.pagedModel.content[*].jobParameters.parameters['foo2'].value", contains(0)))
