...
public class FreeMarkerConfiguration {
    ...
    @Bean
    public FreeMarkerConfigurationFactoryBean springBatchAdminFreeMarkerConfiguration() {
        FreeMarkerConfigurationFactoryBean freemarkerConfiguration = new FreeMarkerConfigurationFactoryBean();
        freemarkerConfiguration.setTemplateLoaderPaths("classpath:/templates/");
        // Required to allow references to the "spring.ftl" macro library
        freemarkerConfiguration.setPostTemplateLoaders(new ClassTemplateLoader(FreeMarkerConfigurer.class, ""));
        freemarkerConfiguration.setPreferFileSystemAccess(false);
        freemarkerConfiguration.setDefaultEncoding("UTF-8");
        Map<String, Object> freeMarkerVariables = new HashMap<>();
        freeMarkerVariables.put("menuManager", menuManager());
        freemarkerConfiguration.setFreemarkerVariables(freeMarkerVariables);
        Properties settings = new Properties();
        settings.put("recognize_standard_file_extensions", "true");
        freemarkerConfiguration.setFreemarkerSettings(settings);
        return freemarkerConfiguration;
    }

    @Bean
    public ViewResolver springBatchAdminViewResolver() {
    ...
    private FreeMarkerView parentFreeMarkerView() {
        ...
        fmv.setConfiguration(springBatchAdminFreeMarkerConfiguration().getObject());
        fmv.setExposeSpringMacroHelpers(true);
