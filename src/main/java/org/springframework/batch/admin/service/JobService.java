...
public interface JobService {
	 ...
	 * names returned from {@link #listJobs(int, int)} might be in the repository,
	 * but not be launchable if the host application has no configuration for them.
	 * 
	...
	/**
	 * Launch a job with the parameters provided. If an instance with the parameters
	 * provided has already failed (and is not abandoned) it will be restarted.
	 * 
	 ...
	 * 
	 * @param jobName the name of the job
	 * @return the last parameters used to execute this job or empty if there are
	 *         none
	 * 
	 * @throws NoSuchJobException
	 */
	...
	/**
	 * Launch a job with the parameters provided. JSR-352 supports restarting of
	 * jobs with a new set of parameters. This method exposes this functionality
	 *
	 * @param jobExecutionId the job execution to restart
	 ...
	 * implementations to ensure that the signal is obeyed. In particular, if users
	 * provide a custom {@link Tasklet} to a {@link Step} it must check the signal
	 * in the {@link JobExecution} itself.
	 * 
	 ...
	 * because the process died this is the best way to mark a job as finished with
	 * (as opposed to STOPPED). An abandoned job execution can be restarted, but a
	 * stopping one cannot.
	 * 
	 * @param jobExecutionId the job execution id to abort
	 * @return the {@link JobExecution} that was aborted
	 * @throws NoSuchJobExecutionException
	 * @throws JobExecutionAlreadyRunningException if the job is running (it should
	 *                                             be stopped first)
	 */
	...
	/**
	 * List the {@link JobInstance job instances} in descending order of creation
	 * (usually close to order of execution).
	 * 
	 * @param jobName the name of the job
	...
	/**
	 * Count the number of {@link JobInstance job instances} in the repository for a
	 * given job name.
	 * 
	 * @param jobName the name of the job
	...
	/**
	 * List the {@link JobExecution job executions} for a job in descending order of
	...
	/**
	 * List the {@link JobExecution job executions} in descending order of creation
	...
	/**
	 * Get the {@link StepExecution step executions} for a given job execution (by
	 * id).
	 * 
	...
	/**
	 * List the {@link StepExecution step executions} for a step in descending order
	...
	/**
	 * Get the names of the steps in a job (or a historical list of recent execution
	 * names if the Job is not launchable).
	 * 
	 * @param jobName the name of the job
